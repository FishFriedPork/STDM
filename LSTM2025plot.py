...
def main():
    # -----------------------------
    # 1. Read AQI data for the specified date (June 1, 2025) from the new data file
    # -----------------------------
    aqi_file = r"C:\Users\于嘉诚\Desktop\STDM\LSTM_Predict.xlsx"
    # Use skipfooter parameter to skip the last line ("Evaluation Metrics"), ensuring that only data up to 2025/6/1 is read.
    df_all = pd.read_excel(aqi_file, skipfooter=1, engine='openpyxl')

    # Convert the "Date" column to datetime and filter data for June 1, 2025.
    ...
    if df_target.empty:
        print("Data for June 1, 2025 not found. Please check the format of the 'Date' column in the Excel file.")
        return

    # Organize AQI data into a mapping: station -> AQI (all columns except 'Date' are station data)
    ...
    # -----------------------------
    # 2. Read station geographic coordinates (remains unchanged)
    # -----------------------------
    ...
    if df_merged.empty:
        print("Merged data is empty. Please check if the station names match.")
        return

    # -----------------------------
    # 3. Read the Beijing region boundary data
    # -----------------------------
    ...
    # -----------------------------
    # 5. Mask the interpolation result, keeping only data within the Beijing region
    # -----------------------------
    # Using the new union_all() method instead of the deprecated unary_union
    mask = contains(beijing.union_all(), mesh_lon, mesh_lat)
    grid_idw_masked = np.where(mask, grid_idw, np.nan)

    # -----------------------------
    # 6. Plot and save the Beijing AQI interpolation map
    # -----------------------------
    ...
    # Save the image to the specified folder with the name LSTM2025plot.png
    output_path = r"C:\Users\于嘉诚\Desktop\STDM\LSTM2025plot.png"
